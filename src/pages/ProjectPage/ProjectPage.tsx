  ...
  return (
    <div className='container max-w-6xl mx-auto'>
      ...
      <div className='mt-2 overflow-y-auto'>
        <ul className='flex flex-row gap-2 pb-4 mb-2 overflow-y-scroll text-white bg-red-400 min-h-[50vh]'>
          {projectData.state.map((state, idx) => (
            <li key={idx} className='flex flex-col gap-2 justify-between p-2 border min-w-[13rem] bg-slate-800 rounded'>
              <article className='stateContent'>
                <EachState {...state} />
                <ul className='flex flex-col gap-2 py-4 mb-2 overflow-x-scroll max-h-[30vh]'>
                  {!ticketData
                    ? <Loading />
